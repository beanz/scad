...
module cylinder_stack(r, h, fn = 30, col = [0.8, 0.8, 0.8]) {
  translate([0, 0, -h]) {
    color(col) cylinder(r = r, h = h, $fn = fn);
    children();
  }
}

module fin_stack(r1, h1, r2, h2, n) {
  color([0.8, 0.8, 0.8]) for (i = [ 0 : n-1 ]) {
    translate([0, 0, -h1-i*(h1+h2)]) {
      cylinder(r = r1+0.2*i, h = h1, $fa = 5);
    }
    translate([0, 0, -h2-(h1+i*(h1+h2))]) {
      cylinder(r = r2, h = h2, $fa = 5);
    }
  }
...
module e3d_v6_3mm_bowden() {
  cylinder_stack(r = e3d_clamp_d/2, h = e3d_clamp_top_h)
    cylinder_stack(r = e3d_clamp_mid_d/2, h = e3d_clamp_mid_h)
    cylinder_stack(e3d_clamp_d/2, e3d_clamp_bottom_h)
    fin_stack(8/2, 1.5, e3d_clamp_d/2, 1, 1)
    fin_stack(8/2, 1.5, e3d_fin_d/2, 1, 11)
    cylinder_stack(r = e3d_heat_break_d/2, h = e3d_heat_break_h) {
      ...
      translate([0,0, -e3d_heat_block[2]]) color("gold") {
        cylinder_stack(r = 5/2, h = e3d_nozzle_gap)
        cylinder_stack(r = e3d_nozzle_hex_r, h = e3d_nozzle_hex_h,
                         fn = 6, col = "gold") {
          translate([0, 0, -2]) {
